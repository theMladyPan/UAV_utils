...
void Remote::update()
{
    ...
    ESP_LOGD("Remote", "Desired orientation: %f, %f, %f", _desired_orientation[0], _desired_orientation[1], _desired_orientation[2]);
    ESP_LOGD("Remote", "Desired throttle: %f", _desired_throttle);
    _svc = pulse_width_to_throttle(_ibus.readChannel(4));
    _vrb = pulse_width_to_throttle(_ibus.readChannel(5));
    ESP_LOGD("Remote", "Switch: %f, Potentiometer: %f", _svc, _vrb);
}
